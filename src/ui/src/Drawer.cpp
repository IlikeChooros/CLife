...
Drawer::Drawer(
    ...
    size_t pixelRows, size_t pixelCols) : width(width), height(height), pixelRows(pixelRows), pixelCols(pixelCols), _callback(do_nothing)
{
...
void Drawer::showImages(const data::data_batch &data)
{
    ...
    while (window.isOpen())
    {
        ...
        window.clear();
        loadPixels(data[index].input);
        _drawPixels();
        window.display();

        index = index > data.size() ? 0 : index + 1;
    }
}
