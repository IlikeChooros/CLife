    ...
    bool allocate)
{
    ...
    if (allocate){
        noisy.reset(new data::data_batch(data->size()));
    }
    else{
        noisy = std::unique_ptr<data::data_batch>(data);
    }
    ...
    int y)
{
    ...
    for (int r = 0; r < static_cast<int>(rows); ++r)
    {
        for (int c = 0; c < static_cast<int>(cols); ++c)
        {
            ...
            if (new_y >= 0 && new_y < static_cast<int>(rows) && new_x >= 0 && new_x < static_cast<int>(cols))
            {
    ...
    int center_y)
{
    ...
    for (size_t r = 0; r < rows; ++r)
    {
        for (size_t c = 0; c < cols; ++c)
        {
            ...
            if (static_cast<int>(new_y) >= 0 && static_cast<int>(new_y) < static_cast<int>(rows) && static_cast<int>(new_x) >= 0 && static_cast<int>(new_x) < static_cast<int>(cols))
            {
